class App {
    ...
    constructor(appId, appName, appVersion, appOs, ambiente, token, tokenSms) {
        ...
        if(ambiente=="HOMOLOGACAO"){
            ...
            this.urlDom = "https://servidorseguro.cloud/pincars/app/www/";
            this.urlApi = "https://servidorseguro.cloud/pincars/api/";
            this.urlCdn = "https://servidorseguro.cloud/pincars/cdn/";
    ...
    login(idUsuario=null,emailUusario=null,dadosUsuario=null){
...
confirmarFechamento(idAnuncio){
    ...
    */
    filtrotabela(){
                    ...
                    const input = document.getElementById("filtroTabela").value.trim().toUpperCase();

                    // Verifica se o tamanho é igual a 7 (padrão de placas)
                    if (input.length === 7) {
                        // Opcional: validação do formato (placas brasileiras/mercosul)
                        const regex = /^[A-Z]{2,3}[0-9][A-Z0-9][A-Z0-9][0-9]{1,2}$/;
                        if (regex.test(input)) {
                            app.buscarPlaca(input); // Chama a função buscarPlaca
                        } else {
                            aviso("Formato de placa inválido.","Esse formato de placa não é reconhecido pelo nosso aplicativo, verifique as informações inseridas e tente novamente");
                        }
                    }
    ...
    buscarPlaca(placa){
        ...
        console.log("Buscando placa para consulta: "+placa);
